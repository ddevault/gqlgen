...
func TestImportPathForDir(t *testing.T) {
	...
	assert.Equal(t, "git.sr.ht/~sircmpwn/gqlgen/internal/code", ImportPathForDir(wd))
	assert.Equal(t, "git.sr.ht/~sircmpwn/gqlgen/api", ImportPathForDir(filepath.Join(wd, "..", "..", "api")))

	// doesnt contain go code, but should still give a valid import path
	assert.Equal(t, "git.sr.ht/~sircmpwn/gqlgen/docs", ImportPathForDir(filepath.Join(wd, "..", "..", "docs")))

	// directory does not exist
	assert.Equal(t, "git.sr.ht/~sircmpwn/gqlgen/dos", ImportPathForDir(filepath.Join(wd, "..", "..", "dos")))
